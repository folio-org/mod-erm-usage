              ...
              .get(TABLE_NAME_SUSHI_SETTINGS, UsageDataProvider.class, crit, true, getReply -> {
                ...
                if (getReply.failed()) {
                  ...
                  if (dataProviders.size() > 0) {
                    ...
                    Future<UsageDataProvider> udpWithNames = AttributeNameAdder
                      .resolveAndAddAttributeNames(entity, okapiHeaders, vertxContext.owner());

                    udpWithNames.setHandler(ar -> {
                      if (ar.succeeded()) {
                        UsageDataProvider udp = ar.result();
                        PostgresClient postgresClient =
                          PostgresClient.getInstance(vertxContext.owner(), tenantId);
                        postgresClient.save(TABLE_NAME_SUSHI_SETTINGS, udp.getId(), udp,
                          reply -> {
                            try {
                              if (reply.succeeded()) {
                                ...
                                asyncResultHandler
                                  .handle(Future.succeededFuture(PostUsageDataProvidersResponse
                                    .respond201WithApplicationJson(udp,
                                      PostUsageDataProvidersResponse.headersFor201()
                                        .withLocation(
                                          "/usage-data-providers/" + udp.getId()))));
                              } else {
                                asyncResultHandler
                                  .handle(Future.succeededFuture(PostUsageDataProvidersResponse
                ...
                true, false, getReply -> {
                  if (getReply.failed()) {
                    ...
                    if (dataProviders.size() > 0
                      ...
                      try {
                        Future<UsageDataProvider> udpWithNames = AttributeNameAdder
                          .resolveAndAddAttributeNames(entity,
                            okapiHeaders, vertxContext.owner());
                        udpWithNames.setHandler(ar -> {
                          if (ar.succeeded()) {
                            UsageDataProvider udp = ar.result();
                            PostgresClient.getInstance(vertxContext.owner(), tenantId)
                              .update(TABLE_NAME_SUSHI_SETTINGS, udp, new Criterion(idCrit),
                                true, putReply -> {
                                  try {
                                    if (putReply.failed()) {
                            ...
                            asyncResultHandler
                              .handle(Future.succeededFuture(PutUsageDataProvidersByIdResponse
                                .respond500WithTextPlain(ar.cause())));
                          }
                        });
                      } catch (Exception e) {
